...
| C++ batched verifier final head | A/B/A in one guarded process reduced the exact 3-row verifier from 58.894 to 57.037 ms (1.0326x), versus 94.723 ms serial (1.6607x); token parity held and peak was 34.4 GiB | The final mixer and LM head should consume `[1,S,H]` once, but they were only a small part of remaining verifier cost | A warmed 64-token HTTP run still reached just 31.98 tok/s with 22/38 accepted and one fallback; trunk/round cost remains dominant |
