...
| C++ dense-batched layer-major verifier | 67.36 ms for three rows versus 94.72 ms serial; 1.406x; exact token and rollback-continuation parity | Layer-major scheduling, one multi-array evaluation per layer, and batched HC projections improve verifier cost | Depth 2, empty origin, one interleaved A/B/B/A run; still above the retained P73 round cost |
