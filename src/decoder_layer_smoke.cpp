...
int main(int argc, char** argv) {
    if (argc != 3) {
        std::cerr << "Usage: " << argv[0] << " MODEL_DIRECTORY LAYER_INDEX\n";
        return EXIT_FAILURE;
    }
    try {
        const std::size_t layer_index = std::stoull(argv[2]);
        ...
        for (int iteration = 0; iteration < 4; ++iteration) {
            ...
            auto first = layer.forward_decode(first_stream, 9419, state);
            first_values = first.astype(MLX_FLOAT32).to_float32();
            const auto started = std::chrono::steady_clock::now();
            auto second = layer.forward_decode(second_stream, 11, state);
            second_values = second.astype(MLX_FLOAT32).to_float32();
            timings.push_back(std::chrono::duration<double, std::milli>(
                std::chrono::steady_clock::now() - started).count());
        }
        if (first_values.empty() || first_values.size() != second_values.size() ||
            !std::ranges::all_of(second_values, [](const float value) {
                return std::isfinite(value);
            })) {
            throw std::runtime_error("decoder layer output is invalid");
        }
        std::vector<double> warm(timings.begin() + 1, timings.end());
        std::ranges::sort(warm);
        std::cout << "{\"layer\":" << layer_index
                  << ",\"attention\":\""
                  << (layer.uses_full_attention() ? "full" : "linear") << '"'
                  << ",\"ple\":" << (layer.uses_ple() ? "true" : "false")
                  << ",\"first_checksum\":"
                  << std::accumulate(first_values.begin(), first_values.end(), 0.0)
                  << ",\"second_checksum\":"
                  << std::accumulate(second_values.begin(), second_values.end(), 0.0)
                  << ",\"second_cold_ms\":" << timings.front()
                  << ",\"warm_median_ms\":" << warm[warm.size() / 2]
                  << ",\"open_shards\":" << tensors.open_shard_count() << "}\n";
        return EXIT_SUCCESS;
    } catch (const std::exception& error) {
        std::cerr << "qwen38-decoder-layer-smoke: " << error.what() << '\n';
        return EXIT_FAILURE;
    }
}
